...
public class King extends PieceBase {
    final static int kind = 0;
    static BufferedImage[] img =new BufferedImage[3];
    static String skillExp="周囲8マスを攻撃";
    static int skillMax=3;

    static{
        try {
            img[0] = ImageIO.read(new File("img/img_chara/king.png"));
            img[1] = ImageIO.read(new File("img/img_chara/king_enemy.png"));
            img[2]= ImageIO.read(new File("img/img_chara/king_chosen.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public int skill(Board board, Piece piece,int skillPhase) {//駒のスキルを使った時の処理
        for(int[] p:PP.around(piece.getPosition())){
            if(board.getPosition()[p[0]][p[1]]>=100){
                int n=board.getPosition()[p[0]][p[1]]-100;
                board.getEnemyPieces().get(n).skillSet(-1);
                board.getOutside(1).addPiece(board.getEnemyPieces().get(n));
                board.getPosition()[p[0]][p[1]]=-1;
    ...
    public boolean skillCheck(Board board, Piece piece){
        ...
        for(int[] p:PP.around(piece.getPosition())){
            if(board.getPosition()[p[0]][p[1]]>=100)return true;
        }
        return false;
    }

    public PieceBase copy(){
        PieceBase p=new King();
        return p;
    }
